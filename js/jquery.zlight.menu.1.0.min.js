;(function($,window,document,undefined)
{
    ...
    {
        // this.element.css({'height':this.op.height,'line-height':this.op.height,'background-color':this.op.mainColor});
        ...
        if(this.active){
            ...
            this.subLinks.css({'color':this.op.fontColor,'font-size':this.op.fontSize,'border-top-color':this.op.dropBorderDarkColor,'border-bottom-color':this.op.dropBorderLightColor,'line-height':this.op.subMenuHeight});
            this.mobileSelect.fadeTo(0,0).css({'height':this.op.height,'line-height':this.op.height});
            this.mobileSpan.css({'line-height':this.op.height,'color':this.op.lablColor,'font-size':this.op.lablFontSize});
            ...
            Zmenu.prototype.createMobile=function(ele){this.mobileSelect.append('<option>'+this.mobileTxt+'</option>');
            this.allLinks.each(function(indx,element){var hrf=$(element).attr('href'),txt=$(element).text(),hclass=$(element).parents('.zlight-submenu').length,tr='';
            if(hclass>0)
            {
            ...
            {
                this.mobileSelect.on('change.zmenuChange',function(){location=this.options[this.selectedIndex].value});
                this.li.add(this.subli).on('mouseenter mouseleave.zmenuEnter',function(event)
                {if(
                    event.type==='mouseenter'){$(this).css('background-color',ele.op.secondColor).children('a').css('color',ele.op.activeFontColor)
                    ...
                    if($(this).hasClass('zlight-active')===false)
                    {
                        ...
                        if(this.op.transform==='on')
                        {
                            $(window).on('scroll.ZlightScrolling',function()
                             {
                            if($(document).scrollTop()>=ele.op.transformBreak&&ele.counter===0) //浮动事件
                            {
                                ele.element.css({'background-color':'black'});
                                ele.element.css({'position':'fixed','top':'-'+ele.mtop+'px','left':'0','margin':'0'}).stop(0,0).animate({'top':'0'},300,'swing');
                                ele.counter=1}
                                else if($(document).scrollTop()<ele.op.transformBreak)  //停止浮动
                                {
                                    ele.element.css({'background-color':''});
                                    ele.element.css({'position':'relative','margin':'','padding':''});
                                    ele.counter=0}})}};
                                    Zmenu.prototype.init=function(ele)
                                    {   
                                        this.variables(ele);
                                        this.setStyles(ele);this.createMobile(ele);
                                        this.change(ele)
                                    };
                $.fn[pluginName]=function(options)
                {
                    return this.each(function(){if(!$.data(this,'plugin_'+pluginName)){$.data(this,'plugin_'+pluginName,new Zmenu(this,options))}})}})(jQuery,window,document);